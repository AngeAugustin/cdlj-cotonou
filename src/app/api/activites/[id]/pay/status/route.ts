...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!paymentId?.trim()) {
      return NextResponse.json({ error: "pid ou paymentId requis" }, { status: 400 });
    }
    ...
    if (payment.status === "pending" && payment.fedapayTransactionId != null) {
    ...
    const fresh = await service.findPaiementById(paymentId.trim());
    if (!fresh) {
      return NextResponse.json({ error: "Paiement introuvable" }, { status: 404 });
    }
