...
import { ActualiteEngagementPanel } from "@/modules/actualites/components/ActualiteEngagementPanel";
...
export default function ActualiteStatsPage() {
  ...
  useEffect(() => {
    ...
    if (message) {
      ...
      window.history.replaceState(null, "", `/actualites/${id}/stats`);
    }
  ...
  return (
    <DashboardPageShell
      ...
      description="Vues, likes, commentaires et répartition géographique."
      actions={
        <div className="flex items-center gap-2">
          <Link
            ...
            className={cn(
              ...
              "rounded-xl border-slate-200 hidden sm:inline-flex"
            )}
          >
            Détail
          </Link>
          ...
          <div className="relative flex flex-col gap-4 px-6 py-6 sm:flex-row sm:items-center sm:gap-6 sm:px-8 sm:py-7">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1.5 max-w-2xl text-sm leading-relaxed text-amber-200/80">
                Modérez les commentaires et analysez l&apos;audience de cet article.
              </p>
            </div>
          </div>
        </div>

        <ActualiteEngagementPanel actualiteId={id} onToast={showToast} />
      </div>
    </DashboardPageShell>
  );
}
