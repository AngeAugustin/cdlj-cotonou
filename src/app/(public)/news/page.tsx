...
export default function NewsPage() {
  const featured = NEWS_POSTS.filter((n) => n.featured);
  const others   = NEWS_POSTS.filter((n) => !n.featured);
  ...
  return (
    <div className="bg-slate-50 min-h-screen py-16">
      <div className="container mx-auto px-4 md:px-8 max-w-7xl">
        ...
        {/* Featured Post */}
        {featured.map((post) => (
          <Link key={post.id} href={`/news/${post.slug}`} className="block group relative rounded-3xl overflow-hidden bg-white shadow-xl shadow-slate-200/50 mb-12 flex flex-col md:flex-row transition-all hover:shadow-2xl">
